...
public class BaseDao<T> implements IBaseDao<T> {
    ...
    @Override
    public List<T> query(BO bo) {
		Criteria criteria = getSession().createCriteria(getClz());
        if (bo != null) {
		    criteria.add(Example.create(bo).enableLike(MatchMode.ANYWHERE));
        }

		if (SystemContext.getPageOffset() != null) {
			criteria.setFirstResult(SystemContext.getPageOffset());
		}
		if (SystemContext.getPageSize() != null) {
			criteria.setMaxResults(SystemContext.getPageSize());
		}

		// name,-name
		if (StringUtils.isNotEmpty(SystemContext.getOrder())) {
			String[] orders = SystemContext.getOrder().replaceAll("\\s+","").split(",");
			for (String o : orders) {
	...
	@Override
	public Long getTotal(BO bo) {
		Criteria criteria = getSession().createCriteria(getClz()).setProjection(Projections.rowCount());
        if (bo != null) {
		    criteria.add(Example.create(bo).enableLike(MatchMode.ANYWHERE));
        }
